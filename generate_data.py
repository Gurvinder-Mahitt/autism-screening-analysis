...
generate_data.py
------------------
Fills the ticket_shop database with realistic fake sales data.

Since we don't have a real shop, this script pretends to be one:
it creates a handful of events, some customers, and then simulates
daily ticket sales leading up to each event — using the dynamic
pricing rule as it goes (same rule you'd use by hand with a calculator).
...
# ---------------------------------------------------------------------
# 1. Connect to MySQL — edit these to match your local setup
# ---------------------------------------------------------------------
DB_CONFIG = dict(
    host="localhost",
    user="root",
...
def main():
    ...
    for name, venue, days_out, seats, base_price in EVENTS:
        ...
        cur.execute(
            ...
            "VALUES (%s, %s, %s, %s, %s)",
            (name, venue, event_date, seats, base_price),
        )
        ...
        while day <= TODAY and sold_so_far < seats:
            ...
            while remaining > 0:
                ...
                cur.execute(
                    ...
                    "VALUES (%s, %s, %s, %s, %s)",
                    (event_id, customer_id, day, qty, price_today),
                )
                total_sales += 1
    ...
    print("\nDone! Data is ready in the 'ticket_shop' database.")
